...
* 互联网数据抓取宝典
* 
*D:\Laurence\workspace\CSSSQL_V3_Formal\CSSSQL_V3_Formal\Report\FormRecycleAnalyseByAsp.aspx.cs
...
*/
	...
	*/
	...
	define("HOST","http://www.songsongni.com/");
	//抓取模式:
	//1、为写缓存！
	//2、为存数据库+写缓存
	//3、3写缓存 + 读数据库抓图片
	...
	//main();  
	get_m_info("http://www.songsongni.com");

/**
**采集数据的要求(针对采集者对该网站的分析)
**1_需要解析网站传递数据的参数名?keywords=&mainbest=0&designerclassid=-1&location=4&sel_city=0&job=2&charges=0&PageSize=12&vType=2&birthday1=0&birthday2=0&AccountID=&PageNO=".$i
**2_了解网站的架构设计方式,点击查看传参的内容 
**
**
**
*/
...
function get_pro_info_count($url){
	...
	$url->noReturn();
	$url->cut('<div class="works-catalogInfo-space">','</div><div class="clear"></div>');
	$content = $url->value_;
...
function get_m_proarea($url){
	...
	$url = get_page_url($url);
	$url->noReturn();
	$url->cut('<div class="title_box">','</div class="title_box">');
	$content = $url->value_;
	$content = iconv("gb2312","UTF-8",$content);
	//echo $content;exit;
...
function get_m_info($url){
	...
	//取出换行字符
	$url-> noReturn();
	$url-> cut('<div class="modulecontent">','<center>');
	//获取裁剪后的内容
...
get_m_info("http://www.china-designer.com/home/137795_1_1.htm");
